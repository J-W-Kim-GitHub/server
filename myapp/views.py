...
def xgboost_prediction(bike_table, weather_table):
    ...
    df = pd.read_sql_query("SELECT * FROM bike_table", engine)
    ...
    for st in stations: #모든 정류장을 반복문으로 실행
        ...
        trained_model = xgb.XGBRegressor().fit(X_train, y_train) # train 데이터를 바탕으로 train된 모델 생성

        predictions = trained_model.predict(X_test)
        mse = mean_squared_error(y_test, predictions)
        rmse = np.sqrt(mse) #X_test 데이터에 대해서 예측하고 성능 검증하기 위해서 rmse사용

        weather_prediction = pd.read_sql_query("SELECT * FROM weather_table", engine) # db에서 예측된 날씨데이터 가져오기
